...
namespace ToonShading
{
    ...
    public class ToonWater : MonoBehaviour
    {
        ...
        private void OnEnable()
        {
            ...
            if (rt == null)
                rt = new RenderTexture(voronoiScale, voronoiScale, 0, RenderTextureFormat.ARGB32);
        }
        ...
        public float GetBouyancy(Vector3 targetPosition)
        {
            if (Application.isPlaying)
            {
                ...
                Texture2D tex = new Texture2D(voronoiScale, voronoiScale);
                tex.ReadPixels(new Rect(0, 0, voronoiScale, voronoiScale), 0, 0);
                Color pixelColor = tex.GetPixel((int)(voronoiScale * positionInBounds.x), (int)(voronoiScale * positionInBounds.y));
